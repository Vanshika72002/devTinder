...
router.get("/view/request/pending",userAuth,async(req,res)=>{
    const pendingRequests=await ConnectionRequestModel.find({$and:[
    ...
    if(pendingRequests.length){
        pendingRequests.forEach(async function(req){
            ...
            res.status(200).send(
                pendingRequestUsersNames.toString()+" sent you connection request");
        })
    }
    else    
...
router.get("/view/connections",userAuth,async(req,res)=>{
    ...
    const connections=await ConnectionRequestModel.find({$and:[{$or:[{fromUserId:loggedInUser._id},{toUserId:loggedInUser._id}]},{status:"accepted"}]})
    if(connections)
